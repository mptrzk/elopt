...
class ComponentCategordy:
  def __init__(self, sym, series):
    self.sym = sym
    self.series = series
...
def lowpass(components):
  R1, R2, C1, C2 = map(lambda x: x.val(), components)
...
def optimize(f_ref, Q_ref):
  ...
  mse = cost(f_ref, Q_ref, components) 
  while changed:
    changed = False
    for c in components:
      c.dec()
      mse2 = cost(f_ref, Q_ref, components) 
      if mse2 < mse:
        mse = mse2
        changed = True
      else:
    ...
    for c in components:
      ...
      if mse2 < mse:
        ...
        c.dec()
    print(mse)
  print(*lowpass(components), components)
optimize(20e3, 0.707)
